...
* **Research interest**: I am currently interested in using Large Language Models (LLM) for automated program repair. Specifically, my research work contains two perspective:
  * **Pre-training phase**: LLMs are pre-trained on large un-labeled corpus to learn how to generate and understand codes. My research work aims at designing new techniques to add human knowledge into raw corpus, which will improve the performance of LLMs on multiple code-related tasks, including program repair.
  * **Inferencing phase**: When using LLMs for program repair, we need to provide LLMs the context information which are related to the bugs that we want to repair. My research work aims at designing new techniques to extract more accurate context information that are rich in knowledge, which will help LLMs repair bugs more effectively and efficiently.<br/>
* **Challenges** that I face:
  ...
  * **Hard to add human knowledge into LLMs**: Adding human knowledge to LLMs should definitely improve its performance, but it is difficult to insert PL knowledge (abstract syntax tree, control/data flow, etc.) into LLMs effectively. If we cannot design a reasonable technique to apply human knowledge to LLMs, it will actually harm the performance of LLMs because more noise are added into training data. My current solution is to keep learning and trying more related techniques in NLP area and to see whether these techniques will work.<br/>
